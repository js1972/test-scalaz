...
object folding {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(124); val res$0 = 
  ...
  9.some.foldLeft(2) {_ + _};System.out.println("""res1: Int = """ + $show(res$1));$skip(41); val res$2 = 
  ...
  Tags.Disjunction.unwrap(Tags.Disjunction(true));System.out.println("""res6: Boolean = """ + $show(res$6));$skip(136); val res$7 = 
  
  //(List(1, 2, 3) foldMap {Tags.Multiplication}: Int) assert_=== 6
  List(true, false, true, true) foldMap {Tags.Disjunction.apply};System.out.println("""res7: scalaz.@@[Boolean,scalaz.Tags.Disjunction] = """ + $show(res$7))}
}
