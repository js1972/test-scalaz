...
object dlist {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(153); 
  ...
  def delmin[A: Order](stream: Stream[A]): Option[(A, Stream[A])] = stream match {
    ...
    case xs =>
      val y = minimumS(xs)
      (y, deleteS(y, xs)).some
  };System.out.println("""delmin: [A](stream: Stream[A])(implicit evidence$4: scalaz.Order[A])Option[(A, Stream[A])]""");$skip(83); 
